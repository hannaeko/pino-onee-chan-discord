  ...
  zeroChanHandler (message) {
    ...
    promiseGet({
      ...
      if (response.statusCode === 404) {
        ...
        message.channel.send(`Do you mean ?\n${suggestions}`)
      } else if (response.statusCode === 301) {
        return promiseGet({
          url: `https://www.zerochan.net${response.headers.location}`,
          query: { s: 'random' }
        })
      }
    }).then((res) => {
      ...
      let imgUrl = elem.find('img').attr('src')

      let breadcrumbs = $('p.breadcrumbs a')
        .map((i, el) => $(el).text()).get()
      breadcrumbs.push($('div#content h1 span').text())
