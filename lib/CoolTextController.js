...
class CoolTextController {
  ...
  figletHandler (message) {
    ...
    let leetRegexStr = Object.keys(leet).map(e => e.replace(/\|/g, '\\|')).join('|')
    let withoutLeet = msg.replace(new RegExp(`(${leetRegexStr})`, 'ig'), m => leet[m])
    ...
    let badRegex = /(yaoi *(>+|sup([éeru]{3,4})?( [aà] )?) *yuri)|(yuri *(<+|inf([éeru]{3,4})?( [àa])?) *yaoi)/i
