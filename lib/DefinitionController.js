...
class DefinitionController {
    ...
    req.get(
      {
        ...
        if (body.list.length == 0)
          return message.channel.send('no result') // No result
        ...
        message.channel.send(this.urbanMsg(body, 0))
          .then(msg => {
            ...
            msg.react(this.emojiName.prev)
              .then(() => msg.react(this.emojiName.next))
  ...
  urbanMsg (body, index) {
    ...
    if (def.definition.length > 1000)
      finalMessage += `${def.definition.slice(0, 1000)}...\n\n`
    else
